...
import { UserCheck, UserPlus, UserMinus, Building2, Home, RefreshCw, Search, Shield, Info } from 'lucide-react';
...
function AppointModal({ show, onClose, onAppoint, allowedScopes }) {
    const [rollNo, setRollNo] = useState('');
    const [scope, setScope] = useState(allowedScopes?.[0] || 'Department');
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState('');

    useEffect(() => {
        if (allowedScopes?.length) setScope(allowedScopes[0]);
    }, [allowedScopes]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err?.error || err?.detail || err?.message || 'Failed to appoint representative');
        } finally {
            setLoading(false);
        }
    };

    const scopeButtons = (allowedScopes && allowedScopes.length > 0)
        ? allowedScopes
        : ['Department', 'Hostel'];
...
export default function AuthorityRepresentatives() {
    const { user } = useAuth();
    ...
    // Determine what scopes this authority can manage
    const authType = (user?.authority_type || '').toLowerCase();
    const isHOD = authType.includes('hod');
    const isMensWarden = authType.includes("men") && authType.includes("warden");
    const isWomensWarden = authType.includes("women") && authType.includes("warden");
    const isWarden = authType.includes('warden');

    const allowedScopes = isHOD
        ? ['Department']
        : isWarden
        ? ['Hostel']
        : ['Department', 'Hostel']; // Admin

    // For multi-scope admins, allow tab filtering; single-scope defaults to their scope
    const [scopeFilter, setScopeFilter] = useState(allowedScopes.length === 1 ? allowedScopes[0] : 'All');

    const load = useCallback(async () => {
        setLoading(true);
        try {
            const data = await adminService.getRepresentatives({ active_only: true });
            setReps(data.representatives || []);
        } catch (err) {
            console.error('Failed to load representatives:', err);
            setError('Failed to load representatives. You may not have permission.');
        } finally {
            setLoading(false);
        }
    }, []);
                    ...
                    <div className="max-w-5xl mx-auto space-y-6">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-0.5">
                                        {isHOD
                                            ? 'Manage department-scope representatives for your department'
                                            : isWarden
                                            ? 'Manage hostel-scope representatives'
                                            : 'Appointed students who can create petitions'}
                                    </p>
                                </div>
                            </div>
                        ...
                        {(isHOD || isWarden) && (
                            <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-800">
                                ...
                                <span>
                                    {isHOD
                                        ? 'As HOD, you can only appoint Department-scope representatives from your department.'
                                        : isMensWarden
                                        ? "As Men's Hostel Warden, you can only appoint male hostel residents as Hostel representatives."
                                        : isWomensWarden
                                        ? "As Women's Hostel Warden, you can only appoint female hostel residents as Hostel representatives."
                                        : 'As Hostel Warden, you can only appoint hostel-scope representatives.'}
                                </span>
                            </div>
                        )}
                        ...
                        {/* Stats — only show relevant scopes */}
                        <div className="flex gap-4 text-sm">
                            {(allowedScopes.includes('Department')) && (
                        ...
                        {loading ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {filtered.map((rep) => (
                                    <RepCard key={rep.id} rep={rep} onRemove={handleRemove} />
                                ))}
                            </div>
                        )}
